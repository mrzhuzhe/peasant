...
struct sbOpLowering : public OpConversionPattern<toy::SbOp> {
  using OpConversionPattern<toy::SbOp>::OpConversionPattern;

  LogicalResult
  matchAndRewrite(toy::SbOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    auto loc = op->getLoc();
    lowerOpToLoops(op, rewriter, [&](OpBuilder &builder, ValueRange loopIvs) {
      ...
      // inner loop.
      auto loadedLhs =
          affine::AffineLoadOp::create(builder, loc, adaptor.getInput1(), loopIvs);
      auto loadedRhs =
          affine::AffineLoadOp::create(builder, loc, adaptor.getInput2(), loopIvs);

      // Create the binary operation performed on the loaded
      // values.
      return arith::MulFOp::create(builder, loc, loadedLhs, loadedRhs);

      //return affine::AffineLoadOp::create(builder, loc, input1, input2);
    });
    return success();
  }
};
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  patterns.add<AddOpLowering, ConstantOpLowering, FuncOpLowering, MulOpLowering,
               PrintOpLowering, ReturnOpLowering, TransposeOpLowering, sbOpLowering>(
      &getContext());
