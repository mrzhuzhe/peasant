...
#include "hello-verify.h"
#include "hello-verify.skel.h"
...
int main()
{
	...
	skel = hello_verify_bpf__open_opts(&opts);
	if (!skel) {
		printf("Failed to open BPF object\n");
		return 1;
	}

	err = hello_verify_bpf__load(skel);
	...
	if (err) {
		...
		hello_verify_bpf__destroy(skel);
		return 1;
	}
	...
	err = hello_verify_bpf__attach(skel);
	if (err) {
